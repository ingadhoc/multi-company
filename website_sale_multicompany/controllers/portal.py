...
class PaymentPortal(portal.PaymentPortal):
    ...
    def payment_method(self, **kwargs):
        ...
        if "sale_order_id" in kwargs and "access_token" in kwargs:
            try:
                order_sudo = self._document_check_access(
                    "sale.order", int(kwargs.get("sale_order_id")), access_token=kwargs.get("access_token")
                )
        ...
        tokens_sudo = (
            request.env["payment.token"].sudo()._get_available_tokens(None, partner_sudo.id, is_validation=True)
