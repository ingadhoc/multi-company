...
class ResCompany(models.Model):
    ...
    @tools.ormcache(
        ...
        "self.name",
        "self.short_name",
        "self.env.context.get('no_company_sufix')",
    )
    def get_company_sufix(self):
        ...
        condition = (
            len(self) != 1
            or self.env.context.get("no_company_sufix")
            or not self.env.user.has_group("base.group_multi_company")
        )

        return "" if condition else f" ({self.short_name or self.name})"
