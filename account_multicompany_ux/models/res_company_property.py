...
class ResCompanyProperty(models.Model):
    ...
    def init(self):
        ...
        cr.execute(sql.SQL("CREATE OR REPLACE VIEW {} AS ({})").format(sql.Identifier(self._table), sql.SQL(query)))
    ...
    def _get_record(self):
        ...
        if not property_field or not active_id or not active_model:
            _logger.warning("Could not get property record from context %s" % context)
            return False
    ...
    def _compute_property_credit(self):
        for record in self:
            if record.property_field == "credit":
                record.property_credit_id = record._get_property_value()
            else:
    ...
    def _inverse_property_credit(self):
        for rec in self:
            rec._set_property_value(rec.property_credit_id)
